    ...
    with torch.no_grad():
        ...
        for batch in test_dataloader:
            ...
            if task_name == 'sst':
                ...
                labels_text = tokenizer.batch_decode(labels.tolist(), skip_special_tokens=True) # Double check
                # print('labels_text:', labels_text)
                # Append to global lists
                pred_labels_text = [label.strip().lower() for label in pred_labels_text]
                # print('pred_labels_text:', pred_labels_text)
                ...
                # Append raw tensor predictions and labels
            ...
            if task_name == 'sst':
                ...
                batch_accuracy = exact_match_accuracy(pred_labels, labels, task_type=task_name)
        ...
        progress_bar.set_postfix(overall_accuracy=overall_accuracy, precision=precision, recall=recall, f1=f1)
        ...
        # Serialize annotations, including handling strings or TokenSpan objects
        if intervention_type == 'original':
            serialized_annotations = [
                {
                    "token_spans": [
                        ...
                        for span in annotation["token_spans"]  # Process each TokenSpan
                    ],
                    "reduction_percentage": annotation["reduction_percentage"]  # Include reduction percentage
                }
            ...
            serialized_annotations = [
                {
                    "token_spans": [
                        ...
                        for span in annotation  # Process each TokenSpan
                    ],
                    "reduction_percentage": reduction_percentage  # Include reduction percentage
                }
        ...
        if task_name == 'sst':
            correct_predictions = [
                ...
                if lbl.strip().lower() == pred.strip().lower() # could have the paading token at the start
            ]
        else:
            correct_predictions = [
                {
        ...
        if save_results:
            ...
            if intervention_type == 'original':
                ...
                replacement_percentage = str(replacement_percentage * 100).replace('.', '_')  # Convert to string and remove decimal point
                save_path = save_path or f"{directory}/{model_name}_{task_name}_{intervention_type}_seed_{seed}_rep_{replacement_percentage}_test_results.json"
                metrics_save_path = save_path.replace('test_results.json', 'metrics.json')

            print(f"Saving correct predictions to {save_path}")
            with open(save_path, "w") as f:
                json.dump(correct_predictions, f, indent=4)
        # if they are tensors, convert them to item
        ...
        # return overall_accuracy, precision, recall, f1, len(correct_predictions), save_path
